...
/**
 * Captures sensor data from an {@link SensorUpdater}.
...
public class SensorCapturer extends Observable implements Observer
{
     ...
     * @param devInfo Needed to obtain SensorUpdater and info about the device.
     */
    public SensorCapturer(RemoteDeviceInfo devInfo)
    {
        try
        {
            deviceName = devInfo.getDevice().getName();
            deviceID = devInfo.getDevice().getID();
        }        
    ...
    /**
     * Returns the SensorUpdater used in the capture.
     * @return SensorUpdater used in the capture.
     */
    public SensorUpdater getUpdater()
    {
        return updater;
    }
