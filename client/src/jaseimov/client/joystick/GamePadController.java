...
// Santiago Peñate Vera 2010, modification for no controler mode
...
public class GamePadController
{
  ...
  public boolean AnyGamePadFound = false;
  ...
  { //constructor
    ...
    if (cs.length == 0)
    {
      System.out.println("[GamePadController]:No controllers found");
      //System.exit(0);
    }
    else
    {
      System.out.println("[GamePadController]:Num. controllers: " + cs.length);

      // get the game pad controller
      controller = findGamePad(cs);
      if (controller != null)
      {
        System.out.println("[GamePadController]:Game controller: " + controller.getName() + ", " + controller.getType());
   ...
   */
  {
    ...
    if (i == cs.length)
    {
      System.out.println("[GamePadController]:No game pad found");
      AnyGamePadFound = false;
      //System.exit(0);
    }
    else
    {
      System.out.println("[GamePadController]:Game pad index: " + i);
   ...
   */
  {
    ...
    if (comps.length == 0)
    {
      System.out.println("[GamePadController]:No Components found");
      //System.exit(0);
    }
    else
    {
      System.out.println("[GamePadController]:Num. Components: " + comps.length);
    }
  ...
  array index, or -1 */
  {
    ...
    for (int i = 0; i < comps.length; i++)
    {
      ...
      {
        System.out.println("[GamePadController]:Found " + c.getName() + "; index: " + i);
        return i;
      }
    }

    System.out.println("[GamePadController]:No " + nm + " component found");
    return -1;
  ...
  buttonsIdx[] with -1's. */
  {
    ...
    for (int i = 0; i < comps.length; i++)
    {
      ...
      {    // deal with a button
        ...
        {
          System.out.println("[GamePadController]:Found an extra button; index: " + i + ". Ignoring it");
        }
        else
        {
    ...
    {
      System.out.println("[GamePadController]:Too few buttons (" + numButtons
  ...
  an arbitrary decision. */
  {
    ...
    {
      System.out.println("[GamePadController]:No Rumblers found");
      rumblerIdx = -1;
    }
    else
    {
      System.out.println("[GamePadController]:Rumblers found: " + rumblers.length);
  ...
  // return the (x,y) analog stick compass direction
  {
    ...
    {
      //System.out.println("[GamePadController]:(x,y) axis data unavailable");
      return NONE;
    }
    else
    {
  ...
  // return the (z,rz) analog stick compass direction
  {
    ...
    {
      //System.out.println("[GamePadController]:(z,rz) axis data unavailable");
      return NONE;
    }
    else
    {
  ...
  // Return the axes as a single compass value
  {
    ...
    {
      System.out.println("[GamePadController]:Unknown (x,y): (" + xc + "," + yc + ")");
      return NONE;
    }
  ...
  // Return the POV hat's direction as a compass direction
  {
    ...
    if (povIdx == -1)
    {
      System.out.println("[GamePadController]:POV hat data unavailable");
      return NONE;
    }
    else
    {
      ...
      { // assume center
        System.out.println("[GamePadController]:POV hat value out of range: " + povDir);
        return NONE;
      }
    }
  ...
  public int get_number_of_buttons()
  {
    int count = 0;

    for (int i = 0; i <= buttonsIdx.length - 1; i++)
    {
      if (buttonsIdx[i] >= 0)
      {
        count++;
   ...
   */
  {
    ...
    {
      System.out.println("[GamePadController]:Button position out of range (1-"
              + NUM_BUTTONS + "): " + pos);
      return false;
    }
